<script lang="ts">
    ...
    function close(e: CustomEvent<MouseEvent>) {
        if (firstClick) {
            firstClick = false
            return
        }
...
<div class:show class="w-full h-full bg-black z-20 bg-opacity-60 absolute top-0 left-0 pointer-events-none flex opacity-0 transition-all duration-300 justify-center items-center">
    {#if show}
        <div use:clickoutside on:clickoutside={close} transition:slide={{duration: 300, axis: "y"}} class="text-white relative w-[70%] h-[80%] border bg-[rgb(51_51_51)]">
